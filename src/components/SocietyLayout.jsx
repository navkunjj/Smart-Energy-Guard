    ...
    <div className="glass-card p-6 rounded-2xl h-full">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-4 text-[10px] uppercase font-bold">
          <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-500"></div> <span className="text-gray-400">Normal</span></div>
          <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-rose-500"></div> <span className="text-gray-400">Theft</span></div>
        </div>
      </div>
      ...
      <div className="relative flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12 py-6 md:py-10">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <p className="mt-3 md:mt-4 text-[10px] md:text-xs font-black text-white uppercase tracking-tighter text-center line-clamp-2 w-20 md:w-24">Main Pole</p>
          <p className="text-[10px] text-blue-400 font-bold">{readings.mainLine}A</p>
        </div>

        {/* Connection Lines Container (Desktop) */}
        <div className="hidden md:block absolute left-20 right-20 top-1/2 h-0.5 bg-gradient-to-r from-blue-500/50 via-gray-700 to-transparent"></div>
        
        {/* Connection Lines (Mobile) */}
        <div className="md:hidden w-0.5 h-8 bg-gradient-to-b from-blue-500/50 to-gray-700"></div>
          ...
          {houses.map((house) => {
            ...
            return (
              <div key={house.id} className="relative flex flex-col items-center">
                ...
                <div className="hidden md:block absolute -top-12 left-1/2 w-0.5 h-12 bg-gray-700"></div>
                ...
                <div className={`w-full relative z-10 p-4 md:p-6 rounded-2xl transition-all duration-500 border-2 ${
                  ...
                  <div className="flex flex-row md:flex-col items-center gap-4 md:gap-0">
                    <Home size={28} className={`${theft ? 'text-rose-500' : 'text-gray-400'} shrink-0 md:mb-4`} />
                    <div className="text-left md:text-center flex-1">
                      <p className="text-[11px] md:text-xs font-bold text-white mb-0.5 md:mb-1">{house.label}</p>
                      <p className={`text-xs md:text-sm font-black ${theft ? 'text-rose-400' : 'text-emerald-400'}`}>
      ...
      <div className="mt-6 md:mt-8 pt-4 md:pt-6 border-t border-white/5 grid grid-cols-2 gap-3 md:flex md:flex-wrap md:justify-center">
        <div className="px-3 py-2 md:px-4 md:py-2 rounded-xl bg-white/5 border border-white/10 flex items-center gap-2">
          <Zap size={14} className="text-yellow-400" />
          <span className="text-[10px] md:text-xs text-gray-400">Voltage:</span>
          <span className="text-[11px] md:text-xs font-bold text-white">{readings.voltage}V</span>
        </div>
        <div className="px-3 py-2 md:px-4 md:py-2 rounded-xl bg-white/5 border border-white/10 flex items-center gap-2">
          <Activity size={14} className="text-blue-400" />
          <span className="text-[10px] md:text-xs text-gray-400">Total Load:</span>
          <span className="text-[11px] md:text-xs font-bold text-white">{readings.totalPower}kW</span>
        </div>
      </div>
    </div>
  );
