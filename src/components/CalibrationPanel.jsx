    ...
    <div className="glass-card p-6 rounded-2xl relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] opacity-40 mt-0.5 uppercase tracking-widest">
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 w-full sm:w-auto
            ${isRunning
              ...
              : 'bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/30 cursor-pointer'
            }`}
        >
          {isRunning
      ...
      {(isRunning || phase === 'done') && (
        <div className="mb-5 relative z-10">
          <div className="flex items-center justify-between mb-2">
            <span className={`text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 ${phaseColors[phase] || 'opacity-40'}`}>
              {isRunning && <Loader2 size={10} className="animate-spin" />}
              {phase === 'done' && <CheckCircle2 size={10} />}
              {phaseLabels[phase] || ''}
            </span>
            <span className="text-[10px] font-black opacity-40">{progress}%</span>
          </div>
      ...
      {hasOffsets && (
        <div className="relative z-10">
          <p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 mb-3 flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-5 gap-2">
            {Object.entries(offsets).map(([key, val]) => (
              <div
                key={key}
                className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-xl px-3 py-2.5 hover:bg-[var(--card-border)] transition-colors flex flex-col items-center sm:items-start"
              >
                <p className="text-[9px] opacity-40 uppercase font-bold tracking-wider mb-1">
                  {key === 'mainLine' ? 'Main' : key === 'house1' ? 'H-1' : key === 'house2' ? 'H-2' : key === 'house3' ? 'H-3' : 'Volt'}
                </p>
                <p className={`text-xs sm:text-sm font-black ${val >= 0 ? 'text-emerald-500' : 'text-rose-400'}`}>
                  {val >= 0 ? '+' : ''}{val}
                </p>
              </div>
            ))}
      ...
      {!isRunning && phase !== 'done' && !hasOffsets && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-[var(--card-bg)] border border-[var(--card-border)] relative z-10">
          <FlaskConical size={20} className="opacity-40 shrink-0" />
